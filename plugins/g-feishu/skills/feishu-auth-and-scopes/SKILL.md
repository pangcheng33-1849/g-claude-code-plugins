...
1. 运行 `resolve-token`，从 JSON 输出中提取 `access_token`
2. 将 token 通过 `--user-access-token` 或 `--tenant-access-token` 参数显式传给下游 skill 脚本

不要尝试写环境变量或直接读取缓存文件。每次需要 token 时调用 `resolve-token`，它会自动处理缓存和刷新。
...
- 如果当前环境有飞书发送消息的能力（feishu-channel 或 feishu-im-workflow），**优先通过飞书发送包含授权链接的交互卡片**
- 如果在终端直接使用，打印 URL 让用户手动打开即可

流程：

1. 运行 `auth-user --scopes <需要的scope> offline_access`，获取授权 URL 和验证码
2. 将授权 URL 和验证码传达给用户（飞书卡片或终端打印）
3. 用户完成授权后，重新运行 `auth-user`（同样参数），脚本自动完成轮询并缓存 token

**飞书授权卡片示例**（通过 feishu-im-workflow 的 `send-message --msg-type interactive` 发送）：
```json
{
...
**注意**：`auth-user` 脚本启动后会阻塞等待用户授权（轮询），首次执行可以先 kill 掉提取 URL，传达给用户后再重新执行。

常用命令：

- `resolve-token`
  统一从环境变量、本地缓存、refresh 或设备授权里解析可用 token。默认输出完整 JSON（含 `access_token`），用 `jq -r '.access_token'` 提取。**这是获取 token 的首选命令。**
- `resolve-token --identity tenant`
  获取 tenant access token
- `auth-user`
...
**重要**：不要直接读取 `~/.feishu-auth-cache/` 下的文件。所有 token 操作都通过上述命令完成。
