...
- user 优先命令先找内部 user token 运行时变量
- tenant 优先命令先找内部 tenant token 运行时变量

如果本轮先通过 Agent Skill `feishu-auth-and-scopes` 拿到了新 token，优先把它交接给运行时环境再继续调用本 skill；只有当前环境不适合持久化环境变量时，才改用显式参数。
