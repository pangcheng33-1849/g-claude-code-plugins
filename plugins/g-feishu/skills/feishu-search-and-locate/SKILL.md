...
- 如果环境明确是应用级搜索自动化，并且应用本身已对目标对象范围有权限，才显式切 tenant token。
- token 获取、缓存、refresh 和 scope 诊断统一交给 `feishu-auth-and-scopes`；本 skill 只消费显式 token 或环境变量。
- 如果本轮先通过 `feishu-auth-and-scopes` 拿到了新 token，优先把它交接给运行时环境再继续执行；只有环境不适合持久化环境变量时，才改用显式参数。
- 若需要把 token 传给下游脚本或 API 调用，优先显式传参；没有显式参数时，会自动读取内部运行时 token 变量。
- `search-user` 当前只支持 user token：
  - 显式传 `--user-access-token`
  - 或读取内部 user token 运行时变量
