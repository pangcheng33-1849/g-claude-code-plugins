...
所有操作通过脚本执行，在 skill 根目录下运行：

```bash
cd <skill_base_dir> && python3 scripts/sandbox_profile.py <command> [args]
```

脚本位于本 skill 的 `scripts/sandbox_profile.py`。必须在项目根目录执行（脚本使用相对路径 `.claude/settings.local.json`）。
