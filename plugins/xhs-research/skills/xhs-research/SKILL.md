...
- 扩展安装完成后，必须把扩展状态页里显示的 token 放入环境变量 `PLAYWRIGHT_MCP_EXTENSION_TOKEN`
...
这样可以绕过每次连接浏览器时的人工确认弹窗，避免采集流程被中断。
...
### 4. 飞书 Skills（可选）
- 如果当前环境里已经有 `feishu-auth-and-scopes`、`feishu-doc-workflow`、`feishu-im-workflow`，则可以直接复用这些 skills
...
2. **通过飞书发送给用户并加急**（仅当 `feishu-im-workflow` 可用时）：
```bash
# 上传截图
...
如果相关飞书 skill 不可用，直接告知用户需要登录并提供截图本地路径。
...
如果当前环境中有 `feishu-doc-workflow`、`feishu-im-workflow`、`feishu-auth-and-scopes` 这些飞书 skills，在本地文件全部生成后，自动执行以下步骤：
...
如果相关飞书 skill 不可用，跳过此阶段，仅输出本地文件。
