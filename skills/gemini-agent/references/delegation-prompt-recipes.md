...
Use these prompts when the parent agent wants a second Gemini session to act like an independent worker with a crisp operating contract.
...
```xml
<task>
Act as an independent Gemini worker for this repository task.
...
```xml
<task>
Continue the existing Gemini session from its current state.
